...
class ExcelConverter {
  ...
  async loadInputFile (filePath: string): Promise<void> {
    ...
    console.log(
      `Loaded worksheet with ${this.worksheet.rowCount} rows and ${this.worksheet.columnCount} columns`
    )
  }

  private findSectionBoundaries (): {
    subitemStart: number
    workContentStart: number
    materialStart: number
  } {
    if (this.worksheet == null) throw new Error('Worksheet not loaded')
  ...
  private async createOutputWorkbook (
    data: SubitemInfo[] | WorkContent[] | MaterialContent[],
    fileName: string
  ): Promise<void> {
  ...
  async convert (inputFile: string, outputDir: string): Promise<void> {
    try {
      ...
      await this.createOutputWorkbook(
        workContent,
        path.join(outputDir, '工作内容、附注信息表.xls')
      )
